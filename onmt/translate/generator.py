...
def build_generator(opt):
    ...
    fields, model, model_opt, gan_g, gan_d = load_test_model(opt)

    generator = TextGenerator.from_opt(
        model, gan_g, gan_d,
        fields, opt, model_opt
    )
    return generator


class TextGenerator(object):
    def __init__(self, model, gan_g, gan_d, max_length, gpu):
        self._gpu = gpu
    ...
    @classmethod
    def from_opt(cls, model, gan_g, gan_d, fields, opt, model_opt):
        src_reader = inputters.str2reader[opt.data_type].from_opt(opt)
        tgt_reader = inputters.str2reader["text"].from_opt(opt)
        max_length = opt.max_length
        return cls(model, gan_g, gan_d, max_length, gpu=opt.gpu)
    ...
    def generate(self, n_sents):
        ...
        memory_bank = fake_hidden

        self.model.decoder.init_state(src, memory_bank, enc_states)

        for step in range(self.max_length):
            dec_out, attns = self.model.decoder(decoder_in, memory_bank, memory_lengths=lengths, step=step)
